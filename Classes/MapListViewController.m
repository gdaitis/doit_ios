...
- (void)loadInfo
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
                   {
                       self.nearestPoints = [self getNearestPoints];
                       dispatch_async(dispatch_get_main_queue(), ^
                                      {
                                          ...
                                          {
                                              NSString *alertString = [NSString stringWithFormat:@"An error occured"];
                                              UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" 
                                                                                              message:alertString
                                                                                             delegate:nil
                                                                                    cancelButtonTitle:@"OK"
...
- (NSMutableArray *)getNearestPoints
{
	NSString *latitude = [NSString stringWithFormat:@"%g", parent.locationManager.location.coordinate.latitude];
	NSString *longitude = [NSString stringWithFormat:@"%g", parent.locationManager.location.coordinate.longitude];		
    ...
    if (!error)
    {
        ...
        NSSortDescriptor *sortDescriptor = [[[NSSortDescriptor alloc] initWithKey:@"distance_meters" ascending:YES] autorelease];
        NSArray *sortDescriptors = [NSArray arrayWithObject:sortDescriptor];
        NSArray *sortedArray;
        sortedArray = [mutableArray sortedArrayUsingDescriptors:sortDescriptors];
